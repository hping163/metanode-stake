...
async function main() {
  ...
  const startBlock = 100; // 替换为实际起始区块
  const endBlock = 99999; // 替换为实际结束区块
  ...
  console.log('MetaNodeStake proxy address:', await stake.getAddress());
  ...
  console.log("MetaNodeStake (proxy) deployed:", await stake.getAddress());
}
